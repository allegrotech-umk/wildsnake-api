...
public class ShowCaseServiceTest {
    ...
    public void getShowCaseItemsShouldReturnItems() throws Exception {
        ...
        List<Product> products = Arrays.asList(new Product("title1", "http://localhost/image1", "description1", 77), new Product("title2", "http://localhost/image2", "description2", 88));
        when(productRepository.findFirst3ByOrderByIdDesc()).thenReturn(products);

        List<ShowcaseItem> expectedItems = Arrays.asList(new ShowcaseItem("title1", "http://localhost/image1", 77), new ShowcaseItem("title2", "http://localhost/image2", 88));
